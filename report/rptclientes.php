...
for ($i = 0; $i < count($header); $i++) {
    $pdf->Cell($widths[$i], 8, $header[$i], 1);
}
$pdf->Ln();
...
foreach ($records as $row) {
    ...
    $pdf->Cell($widths[3], 6, $pdf->convertxt($row['marca']), 1);
    $pdf->Cell($widths[4], 6, $pdf->convertxt($row['precio']), 1); // Asegurarse de que este índice existe
